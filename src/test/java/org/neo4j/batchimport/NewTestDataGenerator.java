...
public class NewTestDataGenerator {
    ...
    public static void main(String...args) throws IOException {
        ...
        String nodeHeader = "Node:ID\tRels\t"+getPropsHeader(propsPerNode, "Property")+"\tLabel:label\tCounter:int\n";
        String relHeader = "Start\tEnd\tType\t"+getPropsHeader(propsPerRel, "Property")+"\tCounter:long\n";
        //System.out.println("Using: TestDataGenerator "+nodes+" "+relsPerNode+" "+ Utils.join(relTypes, ",")+" "+(sorted?"sorted":""));
        ...
        System.out.println("Creating " + nodes + " Nodes and " + relCount + (sorted? " sorted " : "") + " Relationships with [" + propCount +":"+longPropCount+"] took " + seconds + " seconds.");
    }
    
    private static long generate(FileOutputStream nodeFile, FileOutputStream relFile, int nodes, int startNodeId, int relsPerNode, String[] types, boolean sorted) throws IOException {
    	long relCount = 0;
        ...
        for (int node = startNodeId; node < nodes+startNodeId; node++) {
            ...
            if (node % 1000000 == 0)
            	System.out.println(node+" nodes in "+((System.currentTimeMillis()-start)/1000)+ "secs with ["+longPropCount+"] long property strings");
    ...
    static private String[] genDistinctStrings(int count, boolean limit, String[] reference){
    	 ...
    	 while (labels.size() < count){
     		...
     		else tmp = Math.abs((rnd.nextInt() % (10*count)));
