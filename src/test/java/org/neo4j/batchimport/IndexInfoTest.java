...
public class IndexInfoTest
{
    ...
    {
        assertEquals( stringMap( "batch_import.node_index.foo", "exact" ), new IndexInfo( "node_index", "foo",
                "exact", null ).addToConfig( stringMap() ) );
    }

    @Test
    public void testReadFromConfigEntry() throws Exception
    {
        final IndexInfo info = IndexInfo.fromConfigEntry( stringMap( "batch_import.node_index.foo",
    ...
    {
        final IndexInfo info = new IndexInfo( new String[]{"a", "b", "relationship_index", "bar", "fulltext",
                "file"}, 2 );
        assertEquals( "relationship_index", info.elementType );
        assertEquals( "bar", info.indexName );
        assertEquals( "fulltext", info.indexType );
