...
public class IndexInfo
{
    ...
    {
        if ( !(elementType.equals( "node_index" ) || elementType.equals( "relationship_index" )) )
        {
            throw new IllegalArgumentException( "ElementType has to be node_index or relationship_index, " +
                    "but is " + elementType );
        }
        if ( !(indexType.equals( "exact" ) || indexType.equals( "fulltext" )) )
        {
            throw new IllegalArgumentException( "IndexType has to be exact or fulltext, but is " + indexType );
        }
    ...
    {
        if ( !entry.getKey().matches( "^batch_import\\.(node|relationship)_index\\..+" ) )
        {
            return null;
        }
